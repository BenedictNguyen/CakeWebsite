        ...
        fetch(apiUrls.introCentre)
            ...
            .then((data) => {
                if (data && data.length > 0) {
                    // Lọc dữ liệu có id lẻ cho phần Giới thiệu trung tâm
                    const introData = data.filter(item => item.id % 2 !== 0);
                    // Lọc dữ liệu có id chẵn cho phần Tầm nhìn
                    const visionData = data.filter(item => item.id % 2 === 0);

                    if (introData.length > 0) {
                        setIntroContent(introData[0]); // Giới thiệu trung tâm
                    }
                    if (visionData.length > 0) {
                        setVisionContent(visionData[0]); // Tầm nhìn
                    }
                }
            })
            .catch((error) => {
                console.error('Error fetching intro content from backend:', error);
            });
    }, []);
            ...
            <main className="mainContent">
                ...
                <div className="introContent">
                    <div className="textContent">
                        <h2>GIỚI THIỆU VỀ UNIGLOBE</h2>
                        <p>{introContent.content}</p>
                    </div>
                    
                    {/* Tầm nhìn */}
                    <div className="visionColumn">
                        <h3>TẦM NHÌN VÀ GIÁ TRỊ</h3>
